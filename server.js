...
app.get('/about-us', (req, res) => {
  res.render('pages/about-us', { user: req.session.user });
});

app.get('/contact-us', (req, res) => {
  res.render('pages/contact-us', { user: req.session.user });
});
...
app.post('/create-post', upload.fields([{ name: 'image', maxCount: 1 }, { name: 'poster', maxCount: 1 }]), isLoggedIn, async (req, res) => {
  ...
  try {
      ...
      console.error(error);
      res.status(500).render('pages/index', { error: 'Internal server error' });
  }
});
